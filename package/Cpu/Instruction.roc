...
#
# The type states each instruction's capability rather than its spelling: the
# payload admits only the addressing modes that family has, families with a
# single mode carry none at all, and the fused unofficial opcodes name the two
...
#
# Verified by execution: SingleStepTests (256 opcodes x 10,000 cases) and the
# nestest golden log both exercise every mode, cycle count, and penalty here.
#
...
# eighteen cost 140, to catch mistakes the conformance suites already catch by
# executing every opcode.
#
...
# Shape checks. `Instruction` has no equality method, so these match instead of ==.
shaped : U8, (Instruction -> Bool) -> Bool
shaped = |byte, pred| pred(Instruction.lookup(byte))

expect shaped(
	0xA9,
	|i| match i {
		Load(A, Immediate) => Bool.True
		_ => Bool.False
	},
) # LDA #
expect shaped(
	0xA3,
	|i| match i {
		Load(AandX, IndexedIndirect) => Bool.True
		_ => Bool.False
	},
) # LAX (d,X)
expect shaped(
	0x07,
	|i| match i {
		Fused(Shift(LeftArithmetic), Or, ZeroPage) => Bool.True
		_ => Bool.False
	},
) # SLO
expect shaped(
	0x47,
	|i| match i {
		Fused(Shift(RightLogical), Xor, ZeroPage) => Bool.True
		_ => Bool.False
	},
) # SRE is EOR, not AND
expect shaped(
	0x67,
	|i| match i {
		Fused(Rotate(Right), Adc, ZeroPage) => Bool.True
		_ => Bool.False
	},
) # RRA is ADC, not AND
expect shaped(
	0xC7,
	|i| match i {
		Fused(Dec, Cmp, ZeroPage) => Bool.True
		_ => Bool.False
	},
) # DCP
expect shaped(
	0x90,
	|i| match i {
		Branch(Carry, Bool.False) => Bool.True
		_ => Bool.False
	},
) # BCC
expect shaped(
...
expect Instruction.base_cycles(Instruction.lookup(0xBD)) == 4 # LDA abs,X
expect Instruction.penalty(Instruction.lookup(0xBD)) == Bool.True # reads can cross
expect Instruction.penalty(Instruction.lookup(0x9D)) == Bool.False # STA abs,X pays the fixed 5
expect Instruction.base_cycles(Instruction.lookup(0x9D)) == 5
