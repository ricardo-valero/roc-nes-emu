...
Cpu := {
	...
	step = |cpu0|
		if cpu0.jammed {
			...
			r = resolve(cpu1, Instruction.mode(inst))
			pen : U64
			pen = if Instruction.penalty(inst) and r.crossed {
				1
			} else {
				0
			}
			cpu2 = { ..r.cpu, cycles: r.cpu.cycles.plus_wrap(Instruction.base_cycles(inst).to_u64()).plus_wrap(pen) }
	...
	execute : Cpu, Instruction, Operand, Bool -> Cpu
	execute = |cpu, instr, opd, crossed|
		match instr {
			# loads / stores / transfers
			# LAX writes both A and X, so it cannot share the arm below. It has
			# to come first: Load(dst, _) would otherwise cover it.
			Load(AandX, _) => {
				r = load_val(cpu, opd)
			...
			Load(dst, _) => {
				# AandX (LAX) is handled above, but Roc has no union refinement
				# (roc-lang/roc#5504), so `dst` is still typed as the full set
				# here and the catch-all is required even though it is dead.
				member =
					match dst {
						A => Accumulator
						X => X
